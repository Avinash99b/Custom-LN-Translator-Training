...
For every file:
  ...
  3. Send numbered lines to Azure GPT-4o-mini and ask it to identify ranges to remove.
...
def call_lm_studio(numbered_text: str) -> str:
    """
    Send a chat-completion request to Azure OpenAI-compatible endpoint
    and return the raw assistant response string.
    """
    if not API_KEY or API_KEY == "hidden":
        raise RuntimeError(
            "API key not set. Please export LM_STUDIO_API_KEY with your Azure OpenAI key."
        )
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": USER_TEMPLATE.format(numbered_text=numbered_text)},
        ],
    }
...
def parse_removal_response(raw: str, total_lines: int) -> dict:
    ...
    try:
        ...
        return {"ok": False, "remove_ranges": [], "error": f"JSON decode error: {exc}  |  raw={raw[:200]!r}"}

    if not isinstance(obj, dict):
        return {"ok": False, "remove_ranges": [], "error": f"Expected dict, got {type(obj).__name__}"}

    raw_ranges = obj.get("remove_ranges", [])
    if not isinstance(raw_ranges, list):
        return {"ok": False, "remove_ranges": [], "error": f"remove_ranges is not a list: {raw_ranges!r}"}
...
def ensure_working_folders(root: Path):
    ...
    Only copies files that do not yet exist in the working folder.
    """
    for subdir in ("JP", "EN"):
        src_dir = root / subdir
...
def process_file(chapter_name: str, file_path: Path, language: str) -> dict:
    ...
    try:
        ...
        return _build_result(key, chapter_name, language, "ERROR", [], 0, error=err)
    except requests.exceptions.ConnectionError:
        err = "Cannot connect to Azure endpoint."
        return _build_result(key, chapter_name, language, "ERROR", [], 0, error=err)
    except Exception as exc:
        return _build_result(key, chapter_name, language, "ERROR", [], 0, error=str(exc))

    parsed = parse_removal_response(raw_response, len(send_lines))
    if not parsed["ok"]:
        return _build_result(key, chapter_name, language, "ERROR", [], 0, error=parsed["error"])
    ...
    return _build_result(key, chapter_name, language, "CLEANED", remove_ranges, n_removed)


def _build_result(key, chapter, language, status, removed_ranges, removed_line_count, error=None):
    r = {
        "key": key,
        "chapter": chapter,
        "language": language,
...
def print_summary(by_key: dict, found_only: bool = False):
    ...
    if modified:
        ...
        for r in modified:
            ...
            print(
                f"  [{r['language']}] {r['chapter']}  "
                f"— {total_removed} line(s) removed "
                f"({len(ranges)} range(s))"
            )
            if not found_only:
                for rng in ranges:
                    print(f"      lines {rng['start']}–{rng['end']} ({rng.get('reason','?')})")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "non-story content from light novel chapter files using Azure GPT-4o-mini."
        )
    )
    parser.add_argument(
        "folder",
    ...
    file_map = {f"{lang}/{name}": (name, path, lang) for name, path, lang in all_files}
    ...
    if prog_file.exists():
        ...
        if mode == "restart":
            ...
            print(
                f"Re-running {len(err_keys)} error(s) + "
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKERS) as executor:
        ...
        for key in pending_keys:
            ...
            future = executor.submit(process_file, name, path, lang)
        ...
        with tqdm(
            ...
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    result = _build_result(key, name_p, lang_p, "ERROR", [], 0, error=str(exc))
                ...
                tqdm.write(f"  {'✓' if status != 'ERROR' else '✗'} [{lang}] {name}  →  {status}{suffix}")
                pbar.update(1)
