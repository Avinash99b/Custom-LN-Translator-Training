...
MODEL           = "local-model"   # LM Studio ignores this but it must be present
LLM_TIMEOUT     = 180             # seconds per request
MAX_TOKENS      = 2048            # enough for a full removal list
...
def ensure_working_folders(root: Path):
    """
    Create JP-Working and EN-Working by copying from JP-Output / EN-Output.
    ...
    for subdir in ("JP-Output", "EN-Output"):
        src_dir  = root / subdir
        dst_name = subdir.replace("-Output", "-Working")
...
def main():
    ...
    parser.add_argument(
        "folder",
        help="Root novel folder (contains JP-Output / EN-Output sub-folders)",
    )
    parser.add_argument(
        "--found-only",
        action="store_true",
